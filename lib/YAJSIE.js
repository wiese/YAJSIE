...
var YAJSIE = Class.create({
	...
	gettext: function(message1, message2, n) {
		...
		if (Object.isUndefined(translation)) {	// no translation found
			throw "Translation for '" + message1 + "' not found!";
		}

		if (Object.isString(translation)) {	// singular only
			if (Object.isString(message2)) {
				throw "Plural Translation for '" + message1 + "' not defined!";
			}
			return translation;
		}
			...
			if (Object.isUndefined(translation)) {
				throw "Plural form '" + pluralFormId + "' not found for '" + message1 + "'";
			}
			else {
			...
			throw 'Translation format unknown';
		}
	},
	...
	getPluralFormId: function(rule, n) {
		...
		if (
			...
			throw "The given rule '" + rule + "' could not be parsed successfully.";
		}
		...
		try {
			...
			throw "Error evaluating plural rule '" + rule + "'.";
		}
	...
	applyConfigObject: function(configObject) {
		if (!Object.isUndefined(configObject)) {
			...
			throw 'You have to define at least the locale information';
		}
	},
	...
	setLocale: function(locale) {
		if (!Object.isString(locale)) {
			throw "The given locale setting '" + locale + "' is corrupt.";
		}
		this.locale = locale;
